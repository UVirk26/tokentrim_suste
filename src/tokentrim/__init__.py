...
def object(text):
    sc = Compress(model_type='gpt2', lang='en')
    Context=[]
    
    compression_ratios = [0.3,0.4,0.5,0.6,0.7,0.8]
    for cr in compression_ratios:
        context, reduced_content = sc(text, reduce_ratio = cr)
        _, _, bert_score = score([context], [text], lang='en', model_type='bert-base-uncased')
        to = bert_score.item()
        Context.append([cr, context, to])
    Context.sort(key=lambda x: x[2], reverse=True)
    average_to = sum(to for _, _, to in Context) / len(Context)
    filtered_objects = [obj for obj in Context if obj[2] > average_to]
    highest_cr_object = max(filtered_objects, key=lambda x: x[0])
    return highest_cr_object[1]
    ...
    if file_to_process is None:
        while True:
            ...
            if text == 'exit':
                break
            context = object(text)
            print_context_reduced_context(context)
    else:
...
def print_context_reduced_context(context):
